...
import org.epcis.util.gcp.SimplePureIdentityFilter;
import org.w3c.dom.*;
...
public class BSONWriteUtil {
	...
	public static void putQuantityList(Document o, String fieldName, QuantityListType v) throws ValidationException {
		...
		for (QuantityElementType qet : v.getQuantityElement()) {
			...
			if (qet.getQuantity().getValue() != null) {
				quantity.put("quantity", Double.valueOf(qet.getQuantity().getValue().stripTrailingZeros().toString()));
			}
			if (qet.getUom() != null)
	...
	// disposition
	public static boolean isStandardDisposition(String v) throws ValidationException {
		if (v == null)
			return false;
		else if (CBVAttributeWriteUtil.disp.contains(v))
			return true;
		else {
			ValidationException e = new ValidationException();
			e.setStackTrace(new StackTraceElement[0]);
	...
	public static void putSensorElementList(Document o, SensorElementListType v, UnitConverter unitConverter)
		...
		for (SensorElementType set : v.getSensorElement()) {
			...
			for (SensorReportType e1 : set.getSensorReport()) {
				...
				if (e1.getType() != null) {
					if (CBVAttributeWriteUtil.measurement.contains(e1.getType()))
		...
		for (Object elem : obj) {
			if (elem instanceof JAXBElement) {
				...
				if (name.equals("reason")) {
					...
					if (CBVAttributeWriteUtil.er.contains(reason))
			...
			if (obj instanceof Element) {
				...
				for (int nlIdx = 0; nlIdx < nl.getLength(); nlIdx++) {
					...
					if (nl.getLength() == 1 && node instanceof Text) {
						...
						Object convertedValue = CBVAttributeWriteUtil.getCBVCompliantAttribute(qnameKey,
								element.getAttributes(), value);
						if (convertedValue == null)
							convertedValue = convertType(value, element);
						else if (convertedValue instanceof Boolean && !((Boolean) convertedValue))
							continue;
						...
						// skip cbv-compliant attribute
						if (qnameKey.equals("urn:epcglobal:cbv:mda#countryOfOrigin")
								|| qnameKey.equals("urn:epcglobal:cbv:mda#countryOfExport")
								|| qnameKey.equals("urn:epcglobal:cbv:mda#drainedWeight")
								|| qnameKey.equals("urn:epcglobal:cbv:mda#grossWeight")
								|| qnameKey.equals("urn:epcglobal:cbv:mda#netWeight")
								|| qnameKey.equals("urn:epcglobal:cbv:mda#lotNumber"))
							continue;
